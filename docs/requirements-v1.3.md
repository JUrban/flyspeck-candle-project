...
| G4 custom FFI contract | in progress (selected-source and build-recipe calls eliminated; clean executable rebuild and full regression open) | Either a versioned, total ABI with explicit platform trust, or authenticated elimination from source, build recipe, executable, and selected runtime. The current source uses repository-root launch, fail-closed `Sys.command`, offline exact LP preparation, and a static certificate inventory; the preserved frontier executable still contains the historical patch. |
