...
| G2 language/runtime compatibility | in progress (repository inventory projected onto exact 400-node closure: 3,689 findings/329 files; remedies and semantic regressions remain open) | Closed compatibility ledger for required numeric, float, bytes/string, comparison, exception, mutation, module, parser-state, and I/O behavior; differential and negative tests. |
