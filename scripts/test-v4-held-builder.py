...
class V4HeldBuilderTests(unittest.TestCase):
    ...
    def test_slice_remains_below_protocol_and_exec(self) -> None:
        ...
        self.assertIn("SYS_prctl", source)
        self.assertIn("PR_CAP_AMBIENT", source)
        self.assertIn("PR_CAP_AMBIENT_CLEAR_ALL", source)
        self.assertIn("v4_hb_parse_status_capability_rows", header)
        self.assertNotIn("PR_CAPBSET_DROP", source)
        self.assertNotIn("SYS_capset", source)
        self.assertNotIn("PR_SET_NO_NEW_PRIVS", source)
        ...
        clear_ambient_setup = child.index("SYS_prctl", sigprocmask_setup)
        exit_setup = child.index("SYS_exit", clear_ambient_setup)
        ...
        self.assertLess(sigprocmask_setup, clear_ambient_setup)
        self.assertLess(clear_ambient_setup, exit_setup)
